...
int total_nodes(struct s_node *root)
{
	if (!root)
		return 0;
	return (1 + total_nodes(root->left) + total_nodes(root->right)); // note!!!
}
...
int is_split (struct s_node *r, int *split_num, int t_nodes)
{
    ...
    // ???
    //printf("\n222a %d %d", t_nodes, temp);

	temp = is_split(r->left, split_num, t_nodes) + is_split(r->right, split_num, t_nodes) + 1;
...
// https://github.com/Manmeet2018/42_Exam-C-Intermediate/blob/master/level3/can_split/can_split.c
