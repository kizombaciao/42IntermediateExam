...
void sortlen(char **w)
{
	...
	for (i = 0; w[i] != 0; i++)
	{
		...
		{
			l2 = wlen(w[j]);
			if (l1 >= l2) // WOW!  THE EQUALITY REVERSE ABC ORDER!!!
			{
				ftswap(w, i, j);
...
void sortlen(char **words)
{
	...
	while (words[++i])
	{
		l1 = ft_word_len(words[i]);
		j = i;
		while (words[++j])
		{
			l2 = ft_word_len(words[j]);
			if (l1 >= l2)
			{
				ft_swap(words, i, j);
				l1 = ft_word_len(words[i]); // b/c you swapped words!
			}
		}
	}
}
